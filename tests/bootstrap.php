...
// Check if install-dev folder exists, if not, exit with error
if (!is_dir(_PS_ROOT_DIR_ . 'install-dev')) {
    echo "Error: install-dev folder not found. Tests cannot be run without this folder.\n";
    exit(1);
}

// Dynamically find admin folder
$adminDir = null;
$rootDirContents = scandir(_PS_ROOT_DIR_);
foreach ($rootDirContents as $item) {
    if (is_dir(_PS_ROOT_DIR_ . $item) && strpos($item, 'admin') === 0) {
        $adminDir = $item;
        break;
    }
}
...
if (!$adminDir) {
    echo "Error: Admin folder not found. Tests cannot be run without admin folder.\n";
    exit(1);
}
...
define('_PS_ADMIN_DIR_', _PS_ROOT_DIR_ . '/' . $adminDir . '/');
...
// Dynamically load module file based on parent directory name
